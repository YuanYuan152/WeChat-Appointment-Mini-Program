...
class AppAccount(Base):
    ...
    IntakeIsAdult = deferred(Column(Boolean, nullable=True))
    IntakeSignatureUrl = deferred(Column(String(500), nullable=True))
...
class AppCaseRecord(Base):
    ...
    RiskAssessment = deferred(Column(UnicodeText, nullable=True))
    HeaderInfo = deferred(Column(UnicodeText, nullable=True))
    PhotoUrls = deferred(Column(UnicodeText, nullable=True))
    CreatedAt = Column(DateTime, default=func.now(), nullable=False)
    UpdatedAt = Column(DateTime, nullable=True, onupdate=func.now())
...
class AppCaseRecordAmendmentRequest(Base):
    ...
    Status = Column(String(20), nullable=False, default="PENDING")
    RejectReason = deferred(Column(UnicodeText, nullable=True))
    ReviewedBy = deferred(Column(Integer, nullable=True))
    ReviewedAt = deferred(Column(DateTime, nullable=True))
    CreatedAt = Column(DateTime, default=func.now(), nullable=False)
...
class AppCounselorProfile(Base):
    ...
    TargetGroup = deferred(Column(Unicode(500), nullable=True))
    Mode = deferred(Column(Unicode(100), nullable=True))
    Billing = Column(Integer, nullable=False, default=60000)
    FaceBilling = deferred(Column(Integer, nullable=True))
