/** 个案风险评估表配置（与后端 case_record_risk_config.py 保持一致） */
...
export const CASE_RECORD_FIELD_LABELS = {
  subjective: '来访者的主观描述',
  objective: '对来访者客观描述',
...
export const normalizeRiskChoice = (choice: RiskChoice, itemId?: string): RiskChoice => {
  ...
  if (item?.otherChoice === 'C' && (choice === 'E' || choice === 'OTHER')) return 'C'
  if (choice === 'OTHER') return 'E'
  return choice
}
...
export const normalizeRiskAssessment = (raw?: RiskAssessmentData | null): RiskAssessmentData => {
  ...
  for (const item of RISK_ASSESSMENT_ITEMS) {
    ...
    if (val?.choice) {
      base.items[item.id] = {
        choice: normalizeRiskChoice(val.choice, item.id),
        note: val.note ?? '',
      }
    }
  }
...
export const riskAssessmentIsComplete = (data?: RiskAssessmentData | null): boolean => {
  ...
  return RISK_ASSESSMENT_ITEMS.every(item => {
...
export const formatRiskChoiceDisplay = (itemId: string, choice: RiskChoice, note?: string): string => {
  ...
  const item = RISK_ASSESSMENT_ITEMS.find(i => i.id === itemId)
...
export const formatRiskAssessmentText = (data?: RiskAssessmentData | null): string => {
  const normalized = normalizeRiskAssessment(data)
  const lines: string[] = []
  for (const item of RISK_ASSESSMENT_ITEMS) {
    const val = normalized.items[item.id]
    if (!val?.choice) continue
    lines.push(`${item.index}. ${item.label}：${formatRiskChoiceDisplay(item.id, val.choice, val.note)}`)
  }
  return lines.join('\n')
}
